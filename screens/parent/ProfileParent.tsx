...
import { supabase } from '../../lib/supabase';  // Supabase for fetching user profile
...
const ProfileParent: React.FC = () => {
  ...
  const fetchProfile = async () => {
    try {
      ...
      // جلب معلومات الملف الشخصي من Supabase باستخدام Firebase UID
      const { data: profileData, error: profileError } = await supabase
        .from('profiles')
        .select('full_name, phone, email, city, avatar_url')  // استخدام 'city' بدلاً من 'address'
        .eq('id', userId)  // استخدام UID الخاص بـ Firebase
        .single();
      ...
      const formattedProfile: ParentProfile = {
        ...
        address: profileData.city || '',  // استبدال city بـ address
