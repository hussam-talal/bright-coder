...
const ImageRecognitionGames: React.FC<Props> = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      const auth = getAuth(); // استخدم Firebase Auth
      const user = auth.currentUser; // جلب المستخدم الحالي من Firebase

      if (user) {
        setUserId(user.uid); // تعيين معرف المستخدم من Firebase

        // جلب دور المستخدم من جدول profiles في Supabase
        const { data: userProfile, error } = await supabase
          .from('profiles')
          .select('role')
          .eq('id', user.uid) // استخدم معرف المستخدم من Firebase
          .single();
        ...
        if (error) {
          ...
          setUserRole(userProfile.role); // تعيين دور المستخدم
        }
      } else {
        Alert.alert('Error', 'No user session found.');
      }
